...
// B+C가 target보다 작으면 B+C값을 키우기 위해 시작점을 +1해준다.
// 주의점(건너뛰기) : A,B,C는 서로 다른 수이기 때문에 B,C는 A가 되면 안된다.(마찬가지로 B랑 C가 같으면 종료)
// 종료조건 : B랑 C가 1씩 증가감소하다가 같아지면 끝, 또는 A=B+C일경우
...
// [이건 투포인터 풀이하고 읽자.]
// 투포인터의 경우 시작점과 끝점을 B,C로 정하고 시작점과, 끝점을 +1, -1하면서 A를 찾는 방법이며.
// 시작점과 끝점을 옮길 때 B,C가 A인지를 확인하며 건너뛰어 A와의 중복을 피할 수 있다.(B,C가 A인지 확인하는 방법)

// 하지만 이분탐색으로 풀게되면 서로 다른 B,C를 정하고, A를 찾을 때 배열 전체에서 탐색을 해야하는데, 이때 B,C와의 중복을 피하는 예외처리가 어렵다.(A가 B,C인지 확인하는 방법)
// 만약 한다고 하면, 2중 for문으로 B,C를 정하고(B<C조건 필수). A를 이분탐색하며 mid가 B or C의 인덱스이면 start=mid+1을 해줘야한다.(생각해보니 음수가 있을 수 있기 떄문에 무조건 mid+1은 아니긴하네)
